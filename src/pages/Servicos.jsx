...
// ─── Mapa de classes Tailwind por acento ──────────────────────────────────────
const acentoMap = {
  cyan: {
    badge: 'bg-cyan-950/60 border-cyan-700/40 text-cyan-400',
    ...
    detailGlow: 'bg-cyan-500/8',
  },
  blue: {
    badge: 'bg-blue-950/60 border-blue-700/40 text-blue-400',
    ...
    detailGlow: 'bg-blue-500/8',
  },
  teal: {
    badge: 'bg-teal-950/60 border-teal-700/40 text-teal-400',
    iconBg: 'bg-teal-950/80 border-teal-800/50',
    iconColor: 'text-teal-400',
    dot: 'bg-teal-400',
    glow: 'bg-teal-500/10',
    border: 'hover:border-teal-600/60',
    highlight: 'text-teal-400',
    cardBorder: 'border-teal-800/30',
    detailGlow: 'bg-teal-500/8',
  },
  sky: {
    badge: 'bg-sky-950/60 border-sky-700/40 text-sky-400',
    ...
    detailGlow: 'bg-sky-500/8',
  },
  green: {
    badge: 'bg-green-950/60 border-green-700/40 text-green-400',
    iconBg: 'bg-green-950/80 border-green-800/50',
    ...
    detailGlow: 'bg-green-500/8',
  },
  orange: {
    badge: 'bg-orange-950/60 border-orange-700/40 text-orange-400',
    iconBg: 'bg-orange-950/80 border-orange-800/50',
    iconColor: 'text-orange-400',
    dot: 'bg-orange-400',
    glow: 'bg-orange-500/10',
    border: 'hover:border-orange-600/60',
    highlight: 'text-orange-400',
    cardBorder: 'border-orange-800/30',
    detailGlow: 'bg-orange-500/8',
  },
};
...
function DetalheServico({ servico, onVoltar }) {
  ...
  return (
    <motion.div
      ...
      animate="visible"
      exit="exit"
      className="w-full min-h-screen bg-slate-950 text-slate-50 pb-40"
    >
      {/* Grade tecnológica de fundo */}
      <div className="fixed inset-0 bg-[linear-gradient(to_right,#1e293b_1px,transparent_1px),linear-gradient(to_bottom,#1e293b_1px,transparent_1px)] bg-[size:3.5rem_3.5rem] [mask-image:radial-gradient(ellipse_70%_60%_at_50%_10%,#000_60%,transparent_100%)] opacity-20 pointer-events-none z-0" />
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 pt-16 md:pt-20">
        ...
        <motion.div variants={fadeIn} initial="hidden" animate="visible" custom={0.1}>
          <button
            onClick={onVoltar}
            className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-300 text-sm font-medium transition-colors duration-200 mb-10 group"
          >
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.3}>
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-8 mb-6 backdrop-blur-sm">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 bg-green-950/80 border border-green-800/50 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                <HeartPulse className="w-5 h-5 text-green-400" />
              </div>
              <div>
                <span className="block text-xs font-bold text-green-400 uppercase tracking-widest mb-2">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.45}>
          <div className={`bg-slate-900/60 border ${cores.cardBorder} border rounded-2xl p-8 mb-6`}>
            <div className="flex items-start gap-4">
        ...
        <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0.55}>
          <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-8 mb-8">
      ...
      <motion.div
        ...
        className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-6 pt-4 bg-gradient-to-t from-slate-950 via-slate-950/95 to-transparent"
      >
        <div className="max-w-2xl mx-auto flex flex-col sm:flex-row gap-3">
          <button
            onClick={onVoltar}
            className="flex-1 inline-flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-slate-600 text-slate-100 font-bold px-6 py-4 rounded-xl transition-all duration-200 text-sm shadow-lg"
          >
          ...
          <Link
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold px-6 py-4 rounded-xl transition-all duration-200 text-sm shadow-xl shadow-cyan-500/25 hover:shadow-cyan-400/35"
          >
...
function CardServico({ servico, onClick }) {
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      className={`group w-full text-left bg-slate-900/60 border border-slate-800 ${cores.border} rounded-2xl p-7 transition-all duration-300 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500`}
    >
      {/* Ícone */}
      ...
      {/* Dor resolvida */}
      <div className="bg-slate-800/40 border border-slate-700/40 rounded-lg px-4 py-3 mb-5">
    ...
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <section className="relative overflow-hidden">
          ...
          {/* Grade tecnológica de fundo */}
          <div className="absolute inset-0 bg-[linear-gradient(to_right,#1e293b_1px,transparent_1px),linear-gradient(to_bottom,#1e293b_1px,transparent_1px)] bg-[size:3.5rem_3.5rem] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_50%,#000_50%,transparent_100%)] opacity-20 pointer-events-none z-10" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 min-h-[70vh]">
            ...
            >
              {/* Overlay avermelhado */}
              <div className="absolute inset-0 bg-gradient-to-t from-red-950/95 via-red-900/70 to-slate-950/60" />
              <div className="absolute inset-0 bg-slate-950/30" />

              <div className="relative z-10 p-8 md:p-12 pb-12">
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-950/80 border border-red-700/50 text-red-400 text-xs font-bold tracking-widest uppercase mb-5">
                  <AlertTriangle className="w-3 h-3" />
                  O Problema
                </span>
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-extrabold leading-tight mb-4 text-slate-50">
            ...
            >
              {/* Overlay ciano */}
              <div className="absolute inset-0 bg-gradient-to-t from-cyan-950/95 via-cyan-900/60 to-slate-950/50" />
              <div className="absolute inset-0 bg-slate-950/20" />

              <div className="relative z-10 p-8 md:p-12 pb-12">
                <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-cyan-950/80 border border-cyan-700/50 text-cyan-400 text-xs font-bold tracking-widest uppercase mb-5">
                  <Zap className="w-3 h-3" />
                  A Solução
                </span>
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-extrabold leading-tight mb-4 text-slate-50">
                  A Engenharia do{' '}
                  <span className="text-cyan-400">Conforto Perfeito</span>
                </h2>
                <p className="text-slate-300 text-sm md:text-base leading-relaxed max-w-md">
                  Ambientes climatizados com precisão termodinâmica{' '}
                  <strong className="text-cyan-300">impulsionam resultados</strong>. Colaboradores
        ...
        {/* ─── GRID DE SERVIÇOS ────────────────────────────────────────────────── */}
        <section className="py-24 px-6 md:px-12 border-t border-slate-800/60 relative">
          ...
          <div className="max-w-6xl mx-auto relative z-10">
            ...
            >
              <div className="bg-slate-900/60 border border-slate-800 rounded-2xl px-8 py-10 max-w-2xl mx-auto">
                ...
                <Link
                  to="/contato"
                  className="inline-flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold px-8 py-4 rounded-xl transition-all duration-200 shadow-lg shadow-cyan-500/20 hover:shadow-cyan-400/30"
                >
